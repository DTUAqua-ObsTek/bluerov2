...
def rotate_ecef_to_enu(lat: float, long: float, degrees: bool=True):
    if degrees:
        lat = deg2rad(lat)
        long = deg2rad(long)
    return array([[-sin(long), cos(long), 0],
